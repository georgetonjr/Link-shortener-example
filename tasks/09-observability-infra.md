...
# tarefas
- Criar `src/application/shared/request-context.ts` com AsyncLocalStorage para propagar
  o requestId por toda a requisição (controllers, usecases, repositories, infra)
- Criar `src/application/shared/request-id-middleware.ts`:
...
- Atualizar `logger.ts` para injetar automaticamente o `requestId` (via `mixin`) em
  toda chamada de log, sem precisar passar explicitamente por parâmetro
- Incluir `requestId` no corpo das respostas de erro (error-handler.ts)
- Criar pasta `src/infra/` para configuração global de infraestrutura:
...
- Atualizar `server.ts` e demais imports para os novos caminhos
- Adicionar `jest.setup.ts` com env vars padrão de teste, configurado em `jest.config.ts`
  (necessário pois `env.ts` valida variáveis obrigatórias na carga do módulo)
- Testes: middleware de requestId (gera novo id, propaga id recebido via header)
